...
| `ADMIN_API_KEY` | `e305...` | Секретный ключ для авторизации фронтенда (`X-SJK-Key`). |
| `STORAGE_CORS_ORIGIN` | `https://sjk-smart-view.vercel.app` | Домен для разрешения скачиваний. |
...
## Часть 4. Безопасность и Troubleshooting

### 1. Защита API (X-SJK-Key)
Все "тяжелые" запросы (генерация, работа с БД) защищены ключом. 
- Убедитесь, что `ADMIN_API_KEY` на Railway совпадает с тем, что вы используете в запросах.
- Ключ передается в заголовке `X-SJK-Key`.

### 2. Кнопка "Скачать" не работает (CORS)
Если при нажатии на кнопку "Скачать" файл открывается в новой вкладке вместо сохранения:
1. Проверьте настройки CORS в вашем Firebase Storage Bucket.
2. Добавьте разрешение для домена Vercel:
   ```json
   [
     {
       "origin": ["https://sjk-smart-view.vercel.app"],
       "method": ["GET"],
       "maxAgeSeconds": 3600
     }
   ]
